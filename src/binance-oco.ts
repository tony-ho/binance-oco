...
export const binanceOco = async (options: {
    ...
    const cancelOrderAsync = async (symbol: string, orderId: number): Promise<void> => {
        if (!isCancelling) {
            isCancelling = true;
            try {
    ...
    const placeTargetOrderAsync = async (orderAmount: string): Promise<number> => {
        try {
            const response = await binance.order({
                symbol: pair,
                side: 'SELL',
    ...
    const waitForSellOrderFill = (sellOrderId: number): Promise<void> => new Promise((resolve, reject): void => {
        let stopOrderId = sellOrderId;
        let targetOrderId = 0;

        try {
            disconnect = binance.ws.trades(pair, async (trade: Trade): Promise<void> => {
                try {
            ...
            binance.ws.user((msg: Message): void => {
                try {
    ...
    const adjustSellAmountsForCommission = async (commissionAsset: string, stepSize: string): Promise<void> => {
        if (commissionAsset !== 'BNB' || nonBnbFees) {
            try {
                const tradeFee = (await binance.tradeFee()).tradeFee.find((ei: { symbol: string }): boolean => ei.symbol === pair);
                if (tradeFee) {
    ...
    const symbolData = (await binance.exchangeInfo()).symbols.find((ei: { symbol: string }): boolean => ei.symbol === pair);
    if (!symbolData) {
        throw new Error(`Could not pull exchange info for ${pair}`);
    }

    const { filters } = symbolData;
    const { stepSize } = filters.find((eis: { filterType: string }): boolean => eis.filterType === 'LOT_SIZE') as SymbolLotSizeFilter;
    const { tickSize, minPrice } = filters.find((eis: { filterType: string }): boolean => eis.filterType === 'PRICE_FILTER') as SymbolPriceFilter;
    const { minNotional } = filters.find((eis: { filterType: string }): boolean => eis.filterType === 'MIN_NOTIONAL') as SymbolMinNotionalFilter ;
    ...
    if (buyPrice) {
        ...
        if (buyLimitPrice) {
            ...
            const accountBalance = accountInfo.balances.find((ab: { asset: string }): boolean => ab.asset === quoteAsset);
            ...
            const { multiplierUp } = filters.find((eis: { filterType: string }): boolean => eis.filterType === 'PERCENT_PRICE') as SymbolPercentPriceFilter;
    ...
    if (stopPrice) {
        ...
        if (stopLimitPrice) {
            ...
            const currentPrice = await getAveragePrice(pair);
            const { multiplierDown } = filters.find((eis: { filterType: string }): boolean => eis.filterType === 'PERCENT_PRICE') as SymbolPercentPriceFilter;
